...
def extract_30mer_with_ngg(file_path):
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
            content = "".join([line.strip() for line in lines if not line.startswith('>')]).upper()
    except FileNotFoundError:
        print("Not found")
        return []

    pattern = r'(?=([ATCG]{24}[ATCG]GG[ATCG]{3}))'

    matches = re.findall(pattern, content)
    
    return matches

def filter_and_save_csv(results, output_file="./vicrispr_xu2015_all.csv"):
    
    ml_scores = get_ml_score(results)
    ...
    for i in range(len(results)):
        ...
        is_valid = (
            rs2 >= RS2_THRESHOLD and
            GC_MIN <= gc_content <= GC_MAX 
            and mfe_score >= MFE_THRESHOLD
        )
        ...
        if is_valid:
            row = {
                ...
                "Doench et al. 2016 (Azimuth - Rule Set 2)": rs2,
        ...
        row = {
            "Sequence": x,
            "Doench et al. 2016 (Azimuth - Rule Set 2)": ml_scores[i] if i < len(ml_scores) else None,
...
results = extract_30mer_with_ngg("Xu2015.fa")
...
calculate_detailed_metrics("./Xu-2015_Is-Efficient.csv", "./vicrispr_xu2015_all.csv")